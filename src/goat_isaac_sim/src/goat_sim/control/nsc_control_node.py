...
class NSCTesterNode(Node):
    def __init__(self):
        ...
        # Parameters
        self.dt = 1/200
        self.Kp = np.eye(self.n_joints) * 0.5
        self.Kd = np.eye(self.n_joints) * 0.1
        self.Ko = np.eye(self.nv) * 5.0                                    # MOB gain (Base 6 + Joints n)
        self.wheel_Kp_att = 0.5
        self.wheel_Kd_att = 0.1
