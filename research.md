...
> **Q. `oMi`가 정확히 뭔지 궁금합니다. (로컬 → 월드 프레임 변환을 해주는 메소드 같은 건가요?)**
>
...
> **Q. `oMi.act(p)`와 `oMi.rotation @ v`가 뭐가 다른가요?**
>
> SE3 변환 공식: `p_world = R * p_local + t`
>
> - **`oMi.act(p)`** → R × p + t 전체 적용 (회전 + 평행이동)
>   - 사용 대상: **점(position)** — 원점 기준 절대 위치이므로 t를 더해야 월드 좌표가 됨
>   - 예) `com_wheel_L` (바퀴 COM의 위치)
>
> - **`oMi.rotation @ v`** → R × v 만 적용 (회전만, 평행이동 없음)
>   - 사용 대상: **방향벡터(velocity/direction)** — 속도는 기준 원점과 무관하게 방향만 돌리면 됨
>   - 예) `vcom_wheel_L` (바퀴 COM의 속도)
>
> 쉬운 비유: "서울에서 10 km 동쪽에 있는 점" → 좌표계 바꾸면 원점 이동도 반영 → `.act()` /
> "시속 10 km/h 동쪽으로 이동 중" → 방향만 회전시키면 됨 → `.rotation @`
...
> **Q. subtree와 월드 프레임 간의 차이가 궁금합니다.**
>
...
> ```
>
> 같은 공간상의 한 점이지만 기준 좌표계가 달라 숫자가 완전히 다르다.
